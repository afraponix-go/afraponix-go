...
export function BatchTankPicker() {
  ...
  const groupOptions = useMemo(() => {
    ...
    for (const it of items) map.set(it.groupKey, it.groupLabel)
    return [...map.entries()].sort((a, b) => a[1].localeCompare(b[1]))
  }, [items])
  const cropOptions = useMemo(() => [...new Set(items.map((it) => it.cropLabel))].sort((a, b) => a.localeCompare(b)), [items])
  ...
  return (
    <div className="scan-wrap">
      <div className="scan-topline"><Link to="/scan" className="link-btn">‹ Back to scan</Link></div>
      <h2 className="section-title" style={{ marginTop: 0 }}>Choose manually</h2>

      <input
        className="crop-search"
        type="search"
        placeholder="Search batch or tank…"
        value={search}
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="empty">{items.length === 0 ? 'No active batches or tanks found.' : 'Nothing matches.'}</div>
      ) : (
