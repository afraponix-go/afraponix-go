...
// The "sow a batch" form — extracted from SowModal so it can be embedded
// either in that modal (the admin Seedlings page, create or edit) or
// directly on its own page (the operator's simple Seedling capture screen),
// with no duplicated logic between the two.
//
// farmId: the nursery the batch is sown into. systemId: a representative
// system in the farm, only used to load the (per-user) crop list.
export function SeedlingSowForm({ farmId, systemId, seedling, onDone, onCancel }: { farmId: string; systemId?: string; seedling?: Seedling; onDone: () => void; onCancel?: () => void }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['seedlings'] }); onDone() },
