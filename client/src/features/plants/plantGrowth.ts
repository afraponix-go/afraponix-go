...
// ISO-8601 week + week-year (the week's Thursday decides the year, so the
// Dec/Jan boundary lands in the same week-year the growers use).
...
export function recordPlanting(
  ...
  input: {
    ...
    notes?: string
    // Display label + the system's current batch ids, so the batch number
    // follows the "WW/YY · Label" convention and stays unique.
    batch_label?: string
    existing_batch_ids?: string[]
  },
) {
  const batch_id = generateBatchId(input.batch_label ?? input.crop_type, input.existing_batch_ids)
...
export function moveBatch(systemId: string, batchId: string, newGrowBedId: number) {
  // batch_id in the body — "WW/YY · Label" ids contain "/", which Apache blocks
  // in a path segment (even percent-encoded).
  return api(`/data/batch/${systemId}/grow-bed`, {
    method: 'PUT',
    body: { batchId, newGrowBedId },
  })
}
