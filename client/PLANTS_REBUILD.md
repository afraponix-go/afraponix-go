...
- 2026-07-19: 🔧 Course-correction after comparing to the old app (localhost:8000). The rebuilt beds view had used the wrong model. Reworked "Beds & Allocation" → **"Beds"**: bed-centric **plant-fill** view matching the old app's Beds Overview exactly — each bed shows plants, m² used (or verticals for vertical/nft), space left, % full, and its active batches listed under it; per-bed "+ Plant", Edit/Delete bed. Allocation-% dropped from the primary UI (AllocationModal now unused).
  - **Batch aggregation fix (important):** the old app counts a planting as `new_seedlings || count` (6 demo batches store the quantity in `count`, not `new_seedlings`). Updated GET /plants/batches to match, so per-bed and overall plant counts are now correct (DWC Bed 1 = 36 plants, matching the old app; system total 186→323 — the old app's Overview KPI under-counted).
  - New: bedMath.bedFill(); NewPlantingModal gains initialBedId.
  - Still divergent from old app (next): planting-form growth stages (old app has Seedling/Transplant/Vegetative + tracks flowering/fruiting), and crop dropdown cultivars (Lettuce – Batavian, etc.).
